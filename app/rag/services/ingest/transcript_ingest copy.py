...
def apply_sentence_overlap(
    ...
    def _build_overlap_text(chunk_text: str) -> str:
        ...
        for sentence in reversed(sentences):
            ...
            candidate_len = current_len + len(sentence) + len(sep)
    ...
    for chunk in chunks[1:]:
        overlap_text = _build_overlap_text(prev_chunk)
        if overlap_text and not chunk.lstrip().startswith(overlap_text):
...
def ingest_transcript(text: str, summary_metadata: dict):
    ...
    try:
        ...
        raw_chunks = apply_sentence_overlap(raw_chunks, overlap_chars=250)
